...
if curl -sSf $URL; then
    ...
    curl -X POST -H 'Content-Type: application/json' -d '{"text":"The Veras YellowHammer server is down. Please check the logs and inform the infra team. Currently, the server has been automatically restarted. Please ensure that the server is running properly : '$CONTAINER_NAME'."}' $TEAMS_WEBHOOK_URL
	fi