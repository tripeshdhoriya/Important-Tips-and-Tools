    ...
    if ($?) {
        ...
        if ($s_state -eq $service_state) {
			...
			elseif ($dsres -eq "FAILED") {
                ...
                Get-ChildItem "${drive}:\SigmaStream" -recurse -ErrorAction SilentlyContinue | where-object {$_.Length -gt 500*1024*1024} | Sort-Object length | ft length,fullname >> $logpath
