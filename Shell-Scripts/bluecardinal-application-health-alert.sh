...
function validateservice {
    ...
    if [ $s_status -eq 0 ]; then
        ...
        message="FAILED $(/bin/date):WARNING SERVICE($service_name) is restarted"
        sendalert "$message"
...
function servicerestart {
	...
	PIDS=`ps ax | grep bluecardinal-core.jar | grep -v grep | awk '{print $1}'`

	if [ -z "$PIDS" ]; then
	echo "No BlueCardinal engine found to stop"
	else
	#  kill -s TERM $PIDS
	kill -9 $PIDS
	fi
